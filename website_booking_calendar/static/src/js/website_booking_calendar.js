(function(self, $) {
    ...
    self.loadBookings = function(start, end, timezone, callback) {
        ...
        var offset = d.getTimezoneOffset();
        self.session
            .rpc("/booking/calendar/slots/booked", {
                // Start: start.add(offset, 'minutes').format(self.DTF),
                // end: end.add(offset, 'minutes').format(self.DTF),
    ...
    self.viewRender = function(view, element) {
        if (view.name == "agendaWeek") {
            $(element)
                .find("th.fc-day-header")
                .css({cursor: "pointer"})
                .click(function() {
                    var m = moment(
                        $(this).text(),
                        view.calendar.option("dayOfMonthFormat")
                    );
                    if (m < view.start) {
    ...
    self.init = function() {
        ...
        self.$calendar.fullCalendar({
            ...
            timezone: false,
            weekNumbers: true,
            eventSources: [{events: self.loadSlots}, {events: self.loadBookings}],
        ...
        $("#booking-dialog-confirm").click(function() {
            ...
            $form.find("tbody tr").each(function() {
                ...
                deferreds.push(
                    openerp
                        .jsonRpc("/booking/validator", "call", {
                            booking: $(this)
                                .find("select")
                                .attr("name"),
                        })
            ...
            $.when.apply($, deferreds).done(function() {
                if (
                    validated_counter === tr_counter ||
                    $form.find("[name='validation_passed']").val()
                ) {
                    $form.submit();
                }
        ...
        if (self.getParameterByName("expired")) {
            self.warn(
                "Your shopping cart has expired. All the bookings has been cleared"
            );
        }
    };
    ...
    self.eventClick = function(calEvent, jsEvent, view) {
        ...
        if (!$("[name=is_logged]").val()) {
            window.location =
                "/web/login?redirect=" + encodeURIComponent(window.location);
            return;
        }
        var booked = false;
