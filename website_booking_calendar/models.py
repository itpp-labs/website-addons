...
class SaleOrderLine(models.Model):
    ...
    def events_to_bookings(self, events):
        ...
        user_df = ("{} {}".format(lang.date_format, lang.time_format)) if lang else DTF
                    ...
                    for h in bookings[r]:
                        ...
                        if adjacent:
                            for id, p in bookings[r][h]["products"].iteritems():
                                bookings[r][h]["products"][id]["quantity"] += bookings[
                                    r
                                ][hour]["products"][id]["quantity"]
                            del bookings[r][hour]
                            break
                hour_dt += timedelta(hours=MIN_TIMESLOT_HOURS)
        ...
        for k1, v1 in bookings.iteritems():
            for k2, v2 in v1.iteritems():
                for id, product in v2["products"].iteritems():
                    bookings[k1][k2]["products"][id]["price"] = (
...
class SaleOrder(models.Model):
    ...
    def _add_booking_line(self, product_id, resource, start, end, tz_offset=0):
        ...
        for rec in self:
            ...
            values = self.sudo()._website_product_id_change(
                rec.id, product_id, qty=set_qty
            )
            values.update(
                {
            ...
            if (
                not self.env["sale.order.line"]
                .sudo()
                .is_overlaps(int(resource), start.strftime(DTF), end.strftime(DTF))
            ):
                line = (
                    rec.env["sale.order.line"]
                    .sudo()
                    .with_context(tz_offset=tz_offset)
                    .create(values)
                )
            else:
                line = None
        ...
        for order in self.search([("state", "=", "draft")]):
            if order.section_id and order.section_id.code == "WS":
                if (
                    ...
                    + timedelta(minutes=MIN_RESERVATION_MINUTES)
                    < datetime.now()
                ):
                    order.action_cancel()
