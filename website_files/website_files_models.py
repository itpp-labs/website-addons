...
class IrAttachment(models.Model):
    ...
    def _get_website_file_count(self):
        ...
        if self.website_file:
            ...
            count = self.env["ir.ui.view"].search_count(
                ["|", ("arch", "like", '"%s"' % url), ("arch", "like", "'%s'" % url)]
            )
        self.website_file_count = count

    website_file = fields.Boolean("Website", help="Attachment available at website")
    website_file_count = fields.Integer(
        "Number of uses", compute=_get_website_file_count
    )
    ...
    def try_remove_file(self):
        ...
        for attachment in self:
            ...
            for v in views:
                removal_blocked_by[
                    v.id
                ] = v.name  # probably incorrect update on porting
            else:
    ...
    def check(self, mode, values=None):
        if self.ids and mode == "read":
            ids = self.ids[:]  # copy
            self.env.cr.execute(
                "SELECT id,website_file FROM ir_attachment WHERE id = ANY (%s)", (ids,)
            )
...
class Website(models.Model):
    ...
    def file_url(self, record, field="datas", filename_field="datas_fname"):
        model = record._name
    ...
    def search_files(self, needle=None, limit=None):
        name = needle or ""
        res = []
        res = self.env["ir.attachment"].search_read(
            domain=[
                ("website_file", "=", True),
                "|",
                ("datas_fname", "ilike", name),
                ("name", "ilike", name),
            ],
            fields=["datas_fname", "website_file_url"],
            limit=limit,
        )
        return res
