(function() {
    ...
    website.add_template_file(
        "/website_files/static/src/xml/website_files.editor.xml?" + Math.random()
    );
        ...
        start: function() {
            ...
            this.$("#link-file").select2({
                ...
                query: function(q) {
                    ...
                    last = q.term;
                    $.when(self.fetch_files(q.term)).then(
                        function(results) {
    ...
    website.editor.MediaDialog.include({
        start: function() {
            var self = this;
            this.fileDialog = new website.editor.FileDialog(
                this,
                this.editor,
                this.media
            );
            ...
            $('a[data-toggle="tab"]').on("shown.bs.tab", function(event) {
                if ($(event.target).is('[href="#editor-media-file"]')) {
                    ...
                    self.$(
                        "li.search-file, li.previous-file, li.next-file"
                    ).removeClass("hidden");
                    self.$("li.search:has(form)").removeClass("hidden");
                    self.$("li.previous, li.next").addClass("hidden");
                } else {
                    self.$("li.search-file, li.previous-file, li.next-file").addClass(
                        "hidden"
                    );
                }
            });
    ...
    website.editor.FileDialog = website.editor.Media.extend({
        ...
        clear: function() {
            this.media.$.className = this.media.$.className.replace(
                /(^|\s)(img(\s|$)|img-[^\s]*)/g,
                " "
            );
        },
        ...
        fetch_existing: function(needle) {
            ...
            if (needle && needle.length) {
                domain.push(
                    "|",
                    ["datas_fname", "ilike", needle],
                    ["name", "ilike", needle]
                );
            }
            return openerp
                .jsonRpc("/web/dataset/call_kw", "call", {
                    model: "ir.attachment",
                    method: "search_read",
                    args: [],
                    kwargs: {
        ...
        display_attachments: function() {
            ...
            var cur_records = _(records)
                .chain()
                .slice(from, from + per_screen)
                .value();

            this.$(".existing-attachments-files").replaceWith(
                openerp.qweb.render("website.editor.dialog.file.existing.content", {
                    cur_records: cur_records,
                })
            );
            this.parent
                .$(".pager")
                .find("li.previous-file")
                .toggleClass("disabled", from === 0)
                .end()
                .find("li.next-file")
                .toggleClass("disabled", from + per_screen >= records.length);
        },
        ...
        selected_existing: function(link) {
            this.$(".existing-attachments-files .file.media_selected").removeClass(
                "media_selected"
            );
            var $select = this.$(".existing-attachments-files .file")
                .filter(function() {
                    return (
                        $(this)
                            .find(".download")
