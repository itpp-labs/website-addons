...
class SaleOrder(models.Model):
    ...
    def get_shipping_billing(self):
        if not self.buy_way:
            return {"ship_enabled": "1", "bill_enabled": "1"}
        return {
...
class WebsiteConfigSettings(models.TransientModel):
    ...
    def set_default_option(self):
        ...
        for record in self:
            config_parameters.set_param(
                "website_sale_checkout_store.default_option",
                record.default_option or "",
            )

    @api.model
    def get_default_nobill_noship(self, fields):
        nobill_noship = self.env["ir.config_parameter"].get_param(
    ...
    def get_default_bill_noship(self, fields):
        bill_noship = self.env["ir.config_parameter"].get_param(
    ...
    def get_default_bill_ship(self, fields):
        bill_ship = self.env["ir.config_parameter"].get_param(
    ...
    def get_default_nobill_ship(self, fields):
        nobill_ship = self.env["ir.config_parameter"].get_param(
    ...
    def get_default_default_option(self, fields):
        default_option = self.env["ir.config_parameter"].get_param(
            "website_sale_checkout_store.default_option", default="nobill_noship"
        )
