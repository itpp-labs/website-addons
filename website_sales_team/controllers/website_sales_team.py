...
class WebsiteSale(controller):
    ...
    def checkout(self, contact_name=None, email_from=None, phone=None):
        ...
        error = {field for field in ["email_from"] if not post.get(field)}
        ...
        partner_id = partner_obj.search(
            request.cr, SUPERUSER_ID, [("email", "=", values["email_from"])]
        )
        if partner_id:
            ...
            partner_id = partner_obj.create(
                request.cr,
                SUPERUSER_ID,
                {"name": values["contact_name"], "email": values["email_from"]},
            )
        ...
        template_id = ir_model_data.get_object_reference(
            "website_sales_team", "email_template_checkout"
        )[1]
