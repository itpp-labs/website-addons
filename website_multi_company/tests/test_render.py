...
class TestRender(TransactionCase):
    ...
    def test_render(self):
        ...
        for w, text in [(website, VIEW_TEXT), (website2, VIEW_TEXT2)]:
            ...
            result = str(result)
            self.assertFalse('PLACEHOLDER' in result, "Render uses original view, instead of copied one")
            self.assertTrue(text in result, "Rendered template doesn't have updated text. Result:%s\nExpected:%s" % (result, text))
