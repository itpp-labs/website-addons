...
class WebsiteRedirect(models.Model):
    ...
    domain = fields.Char(
        ...
        help="keep empty to apply rules for any domain",
    )
...
class WebsiteRedirectRule(models.Model):
    ...
    pattern = fields.Char(
        "From",
        help="Unix shell-style wildcards. Check https://docs.python.org/2/library/fnmatch.html for details",
        required=True,
    )
...
class IrHttp(models.AbstractModel):
    ...
    def try_redirect(self):
        ...
        if www == "www":
            host = h
        for redirect in (
            request.env["website.redirect"]
            .sudo()
            .search(["|", ("domain", "=", False), ("domain", "=", host)])
        ):
