...
class WebsiteSaleExtended(WebsiteSale):
    ...
    def _check_and_update_child_order(
        self, sale_order, product_id, add_qty, set_qty, force=False, **kw
    ):
        ...
        if not website_id.split_orders or not (
            product_company_id
            and product_company_id != order_company.id
            and (
                product_company_id in website_id.split_orders_companies.ids
                or force
                or (not add_qty and set_qty == 0)
            )
        ):
            return False
        sale_order_child = sale_order.order_child_ids.filtered(
            lambda so: so.company_id.id == product_company_id
        )
        if not sale_order_child:
            ...
            sale_order.write({"order_child_ids": [(4, sale_order_child.id)]})
            _logger.info(
                "website_sale created new child order for company: %s for order: %s",
                product_company_id,
                sale_order.id,
            )
    ...
    def cart_update(self, product_id, add_qty=1, set_qty=0, **kw):
        result = super(WebsiteSaleExtended, self).cart_update(
        ...
        if add_qty:
            ...
            self._check_and_update_child_order(
                sale_order, prod_id, add_qty, set_qty, **kw
            )
    ...
    def cart_update_json(
        ...
        result = super(WebsiteSaleExtended, self).cart_update_json(
    ...
    @http.route(
        ["/shop/split_order_for_daughter_companies/<int:company_id>"],
        type="json",
        auth="public",
        methods=["POST"],
        website=True,
    ...
    def split_order_for_daughter_companies(self, company_id=False):
        ...
        lines = order.order_line.filtered(
            lambda ol: ol.product_id.company_id.id == company_id
        )
        if not lines:
            return False
        result = []
        for line in lines:
            ...
            result.append(
                self._check_and_update_child_order(
                    order, prod, line.product_uom_qty, False, "Force"
                )
            )
        return all(result)
