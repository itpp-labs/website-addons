...
class WebsiteBlogExtended(WebsiteBlog):
    ...
    def blog(self, blog=None, tag=None, page=1, **opt):
        ...
        if request.env.context.get('uid', 0) == SUPERUSER_ID:
            ...
            updated_blogs = blog_env.search(get_domain_from_rule(request.env.ref('website_multi_company_blog.blog_rule_all')))
    ...
    def blogs(self, page=1, **post):
        ...
        if request.env.context.get('uid', 0) == SUPERUSER_ID:
            ...
            updated_posts = post_env.search(get_domain_from_rule(request.env.ref('website_multi_company_blog.post_rule_all')))
            blog_super.qcontext.update({
                'posts': updated_posts,
            })
        return blog_super
