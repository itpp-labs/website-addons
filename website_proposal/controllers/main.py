...
class WebsiteProposal(http.Controller):
    ...
    def post_request_info(self, proposal, message=None):
        body = ("%s<br/>%s") % (message or _("Request info"), self.request_info())
    ...
    def view(self, proposal_id, token=None, message=False, **post):
        ...
        proposal = request.registry.get("website_proposal.proposal").browse(
            request.cr, uid, proposal_id
        )
    ...
    def _render_proposal(self, uid, proposal, message=None):
        record = request.registry.get(proposal.res_model).browse(
            request.cr, uid, proposal.res_id
        )
        values = {
            "proposal": proposal,
    ...
    def accept(self, proposal_id, token=None, signer=None, sign=None, **post):
        ...
        if sign:
            ...
            pdf = request.registry.get("report").get_pdf(
                request.cr, SUPERUSER_ID, [proposal_id], report_name
            )
        ...
        record = request.registry.get(proposal.res_model).browse(
            request.cr, SUPERUSER_ID, proposal.res_id
        )
    ...
    @http.route(
        ["/website_proposal/<int:proposal_id>/<token>/decline"],
        type="http",
        auth="public",
        website=True,
    )
    def decline(self, proposal_id, token, **post):
        ...
        return werkzeug.utils.redirect(
            "/website_proposal/{}/{}?message=2".format(proposal_id, token)
        )

    @http.route(
        ["/website_proposal/<int:proposal_id>/<token>/post"],
        type="http",
        auth="public",
        website=True,
    )
    def post(self, proposal_id, token, **post):
        ...
        if message:
            ...
            self.__message_post(message, proposal, type="comment", subtype="mt_comment")
        return werkzeug.utils.redirect(
            "/website_proposal/{}/{}?message=1".format(proposal_id, token)
        )

    def __message_post(
        self, message, proposal, type="comment", subtype=False, attachments=None
    ):
        request.session.body = message
        cr, uid, context = request.cr, request.uid, request.context
        user = request.registry["res.users"].browse(uid)
        if "body" in request.session and request.session.body:
            request.registry.get(proposal.res_model).message_post(
                proposal.res_id,
    ...
    @http.route(
        ["/website_proposal/template/<model('website_proposal.template'):template>"],
        type="http",
        auth="user",
        website=True,
    )
