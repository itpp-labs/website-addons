...
class ChessGame(models.Model):
    ...
    first_color_figure = fields.Selection(
        [("white", "White"), ("black", "Black")], "Select color for first figure"
    )
    second_color_figure = fields.Selection(
        [("white", "White"), ("black", "Black")], "Select color for second figure"
    )
        ...
        if self.system_status == "Game Over":
            ...
            if turn == "ww" or turn == "bb":
                ...
                if new_result < 0:
                    new_result = 0
                return {
                    "author_time": new_result,
                    "another_user_time": another_user_time,
                }
    ...
    def game_over(self, status, time_limit_id):
        ...
        if len(status) > 0:
            ...
            all_game_f = len(
                self.env["chess.game"].search(
                    [("first_user_id.id", "=", self.first_user_id.id)]
                )
            ) + len(
                self.env["chess.game"].search(
                    [("second_user_id.id", "=", self.first_user_id.id)]
                )
            )
            # all game for second user
            all_game_s = len(
                self.env["chess.game"].search(
                    [("first_user_id.id", "=", self.second_user_id.id)]
                )
            ) + len(
                self.env["chess.game"].search(
                    [("second_user_id.id", "=", self.second_user_id.id)]
                )
            )
            ...
            E_first = 1.0 / (
                1.0 + math.pow(10, ((rating_second - rating_first) / 400.0))
            )
            new_rating_first = rating_first + K_f * (first_game_result - E_first)
            # new rating for second user
            E_second = 1.0 / (
                1.0 + math.pow(10, ((rating_first - rating_second) / 400.0))
            )
    ...
    def refuse_chess_game(self):
        ...
        self.information_chess_game(vals)
        return {
            "domain": "[('system_status', '=', 'Waiting'), ('first_user_id', '!=', %s)]"
            % (self.id),
            "name": "Reload page",
            "view_mode": "tree",
            "view_type": "form",
...
class ChessGameLine(models.Model):
    ...
    def move_broadcast(self, message, game_id):
        ...
        for ps in self.env["chess.game"].search([("id", "=", game_id)]):
            ...
            legal_move = (
                chess.Move.from_uci(data["source"] + data["target"])
...
class ChatMessage(models.Model):
    ...
    date_message = fields.Datetime(
        string="Date message", default=datetime.datetime.now()
    )
    ...
    def message_fetch(self, game_id, limit=20):
        return self.search([("game_id.id", "=", game_id)], limit=limit).sorted(
            key=lambda r: r.id
        )
...
class Tournament(models.Model):
    ...
    def send_tournament_games_data(self, tournament_id):
        ...
        for g in tournament.games:
            games.append(
                {"id": g.id, "player1": g.first_user_id, "player2:": g.second_user_id}
            )
        return games
            ...
            data.append(
                {
                    ...
                    "player1": {
                        "id": game.first_user_id.id,
                        "score": game.first_user_score,
                    },
                    "player2": {
                        "id": game.second_user_id.id,
                        "score": game.second_user_score,
                    },
                    "status": game.status,
