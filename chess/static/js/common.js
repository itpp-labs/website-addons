...
odoo.define("chess.common", function(require) {
    ...
    ChessGame.GameManager = Widget.extend({
        init: function(model_game_id, dbname, uid) {
            ...
            var channel_line = JSON.stringify([
                dbname,
                "chess.game.line",
                [uid, this.game_id],
            ]);
            var channel_game = JSON.stringify([
                dbname,
                "chess.game",
                [uid, this.game_id],
            ]);
            var channel_chat = JSON.stringify([
                dbname,
                "chess.game.chat",
                [uid, this.game_id],
            ]);
            var channel_game_info = JSON.stringify([
                dbname,
                "chess.game.info",
                [uid, model_game_id],
            ]);
            var bus_last = 0;
            Number(storage.getItem("bus_last")) === null
        ...
        on_notification_do: function(channel, message) {
            ...
            if (
                ...
                (channel[1] === "chess.game.line" || channel[1] === "chess.game")
            ) {
                try {
        ...
        create_game: function(message) {
            if (message.system_status === "Active game") {
                window.new_game = new ChessGame.GameConversation(
                    ...
                    function(isConfirm) {
                        if (window.tournament) {
                            window.location.replace(
                                "/chess/tournament/" + window.tournament
                            );
                        } else {
    ...
    ChessGame.GameConversation = Widget.extend({
        init: function(model_game_id, dbname, uid, tournament) {
            ...
            this.lenOldFen = this.OLD_FEN_POSITION.split("/")
                .join("")
        ...
        start: function() {
            ...
            if (!cookie) {
                session
                    .rpc("/chess/game/init", {game_id: self.game_id})
        ...
        call_load_system_message: function(game_id) {
            var self = this;
            session
                .rpc("/chess/game/system_history", {game_id: game_id})
                .then(function(result) {
        ...
        call_load_history: function(game_id, result) {
            var self = this;
            session
                .rpc("/chess/game/history", {game_id: game_id})
        ...
        send_move: function(message) {
            ...
            ref_bw = true;
            session
                .rpc("/chess/game/send/", {message: message, game_id: self.game_id})
                .then(function(result) {
                    if (result === "move") {
                        if (self.game_type === "blitz") {
                            if (game.turn() === "w") {
                        ...
                        if (self.game_type === "limited time") {
                            if (game.turn() === "w") {
                                self.reset(
                                    Math.round(self.another_user_time),
                                    Math.round(self.author_time)
                                );
                                self.clockClicked(0);
                            }
                            if (game.turn() === "b") {
        ...
        updateStatus: function() {
            ...
            if (game.in_checkmate() === true) {
                if (
                    (game.turn() === "b" && self.author_color === "black") ||
                    (game.turn() === "w" && self.author_color === "white")
                ) {
                    setTimeout(function() {
                        swal(
                            {
                ...
                if (
                    (game.turn() === "b" && self.author_color === "white") ||
                    (game.turn() === "w" && self.author_color === "black")
                ) {
                    setTimeout(function() {
                        swal(
                            {
        ...
        game_over: function(status) {
            ...
            session
                .rpc("/chess/game/game_over/", {
                    game_id: self.game_id,
                    status: status_game.toLowerCase(),
                    time_limit_id: not_win_user_id,
                })
                .then(function(result) {
                    if (result) {
                        $("#surrender").hide();
        ...
        onDelFigure: function() {
            ...
            this.OldFenPosition = this.OLD_FEN_POSITION.split("/")
                .join("")
                .replace(/[0-9]/g, "");
            this.NewFenPosition = this.NewFenPosition[0]
                .split("/")
                .join("")
                .replace(/[0-9]/g, "");
            ...
            if (this.lenOldFen !== this.NewFenPosition.length) {
                ...
                if (WhiteArr.length > 0) {
                    var imagesHTML = {
                        P:
                            "<img src='/chess/static/img/chesspieces/wikipedia/wP.png' alt='white pawn'>",
                        B:
                            "<img src='/chess/static/img/chesspieces/wikipedia/wB.png' alt='white bishop'>",
                        N:
                            "<img src='/chess/static/img/chesspieces/wikipedia/wN.png' alt='white kNight'>",
                        R:
                            "<img src='/chess/static/img/chesspieces/wikipedia/wR.png' alt='white rook'>",
                        Q:
                            "<img src='/chess/static/img/chesspieces/wikipedia/wQ.png' alt='white queen'>",
                    };
                ...
                if (BlackArr.length > 0) {
                    var imagesHTML = {
                        p:
                            "<img src='/chess/static/img/chesspieces/wikipedia/bP.png' alt='black pawn'>",
                        b:
                            "<img src='/chess/static/img/chesspieces/wikipedia/bB.png' alt='black bishop'>",
                        n:
                            "<img src='/chess/static/img/chesspieces/wikipedia/bN.png' alt='black kNight'>",
                        r:
                            "<img src='/chess/static/img/chesspieces/wikipedia/bR.png' alt='black rook'>",
                        q:
                            "<img src='/chess/static/img/chesspieces/wikipedia/bQ.png' alt='black queen'>",
                    };
