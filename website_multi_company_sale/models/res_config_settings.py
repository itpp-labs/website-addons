...
class ResConfigSettings(models.TransientModel):
    ...
    website_billing_country_ids = fields.Many2many(
        related="website_id.billing_country_ids",
    ...
    def open_template_user(self):
        current_user_company = (
            self.env["res.users"].browse(self._context["uid"]).company_id
        )
        default_param = (
        ...
        return super(
            ResConfigSettings, self.with_context(force_company=current_user_company.id)
        ).open_template_user()
