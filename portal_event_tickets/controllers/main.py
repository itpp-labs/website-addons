...
class PortalEvent(website_account):
    def _get_archive_groups(
        ...
        for group in (
            request.env[model]
            .sudo()
            ._read_group_raw(domain, fields=fields, groupby=groupby, orderby=order)
        ):
    ...
    def _tickets_domain(self, partner=None):
        ...
        return [("attendee_partner_id", "=", partner.id), ("state", "=", "open")]
    ...
    def ticket_transfer_receive(self, transfer_ticket=None, **kw):
        ...
        if request.httprequest.method == "GET":
            tickets = WebsiteEventController()._process_tickets_details(
                {"nb_register-0": 1}
            )
            values.update(
                {
