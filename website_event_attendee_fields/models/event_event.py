...
class AttendeeField(models.Model):
    ...
    field_id = fields.Many2one(
        "ir.model.fields",
        domain="[('model_id.model', 'in', ['res.partner', 'event.registration'])]",
    )
    ...
    width = fields.Selection(
        ...
        required=True,
        default="4",
        help="Field of a width in the form. One row may have width up to 12",
    )
    ...
    def name_get(self):
        return [
            (r.id, "#{}: {} (width={})".format(r.sequence, r.field_name, r.width))
            for r in self
        ]
